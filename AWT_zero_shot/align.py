...
# MODEL_PATH = "/mnt/petrelfs/zhaozhiyu/zhuyuhan/.clip_ckpt/align-base"
...
def evaluate(val_loader, model, text_features, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        # image_processor = AutoImageProcessor.from_pretrained("/mnt/petrelfs/zhaozhiyu/zhuyuhan/.clip_ckpt/align-base")
        image_processor = AutoImageProcessor.from_pretrained(MODEL_PATH)
        inputs = image_processor(images=torch.cat(images), return_tensors="pt", do_rescale=False)
        inputs['pixel_values'] = inputs['pixel_values'].float().cuda(non_blocking=True)
        ...
        image_weights, text_weights = get_entropy_weight(output / model.temperature)
    ...
    with open(f'./results/align/all.txt', 'a') as f:
...
def main_worker(args):
    ...
    # get models
    # model = AutoModel.from_pretrained("/mnt/petrelfs/zhaozhiyu/zhuyuhan/.clip_ckpt/align-base")
    model = AutoModel.from_pretrained(MODEL_PATH)
    model = model.cuda()
    # tokenizer = AutoTokenizer.from_pretrained("/mnt/petrelfs/zhaozhiyu/zhuyuhan/.clip_ckpt/align-base")
